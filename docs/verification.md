...
- JSON repair regression: pass, `npm run check:json-repair`; covers portrait source-frame correction and generator-facing JSON with structured composition/constraints instead of a prompt-first wrapper.
- Prompt goal coverage: pass, `npm run check:prompt-goal`; 77 contract rules, 13 simulated fidelity cases, 1 simulated JSON generator-readiness case.
- API option check: pass, `npm run check:api-options`.
- Typecheck: pass, `npm run typecheck`.
- Build: pass, `npm run build`.
- Visual history smoke: pass, `npm run smoke:history`, `tmp/browser-tests/pw-visual-history-summary-1782043965412.json`; 14, 35, and 100 record cases, running-history guard, clear-sync guard, popup history rendering, and visible `复制 JSON 提示词` actions passed.
- Bilibili E2E: pass, `npm run e2e:bilibili`, `tmp/browser-tests/pw-e2e-summary-1782043953840.json`; validates capture-region analysis, English copy, structured JSON copy with `task: "image_reconstruction"`, adaptive modules, no prompt-first top-level field, no internal schema leakage, favorite/regenerate/image-pick/local-file/history/language flows.
- Popup/options load diagnostic: pass, `npm run check:extension-pages`; temporary Chromium loaded unpacked `dist`, opened `popup.html` and `options.html`, rendered one root child in each page, and reported no console warnings/errors or page errors after removing modulepreload from the extension shell HTML.
- Content-script page injection diagnostic: pass, `npm run check:content-script`; temporary Chromium loaded unpacked `dist`, opened a local HTTP page, injected `content.js`, rendered `zhijuan-prompt-root`, then reloaded the extension service worker and clicked the old page panel. The page showed the expected reload recovery message and reported zero console/page errors after reload. External Xiaohongshu spot check also passed with `ZHIJUAN_CONTENT_SCRIPT_URL='https://www.xiaohongshu.com/explore?channel_id=homefeed_recommend' npm run check:content-script`; the page had its own login/hydration errors, but extension-related severe console/page errors were zero.
- Automated real-extension UI test: pass, `npm run test:real-extension`, `tmp/browser-tests/manual-real-extension-summary-1782037010430.json`; loaded the built extension in a temporary Chromium profile, saved real BridgeDeck settings with redacted evidence, then analyzed one README page image pick and one local file upload through the actual UI. Both returned `reconstruction_v2`, `en.prompt`, `json_prompt.generation_prompt`, `json_prompt.generation_negative_prompt`, `json_prompt.spatial_dynamics`, `json_prompt.fidelity_priorities`, dynamic `observation_units`, dynamic `reconstruction_priorities`, no hidden Japanese output, and no duplicate recreation-prompt field. This does not replace the separate user-installed refreshed-extension acceptance gate. Page image pick returned 354 English prompt words, 502 JSON generation prompt words, 55 spatial dynamics words, 8 observation units, and 5 reconstruction priorities. Local upload returned 349 English prompt words, 485 JSON generation prompt words, 46 spatial dynamics words, 9 observation units, and 5 reconstruction priorities.
- Computer Use installed-extension simulation: partial, not counted as acceptance. The installed Chrome extension detail page was refreshed from `0.3.4 Frame Lock` to `0.3.4 Prompt Contract`, then a page image selection was triggered from the real browser UI. Result readback was blocked by Computer Use `cgWindowNotFound`, so this only proves refresh and trigger, not successful user-installed analysis output.
- Installed Chrome extension DOM evidence: visible-history prompt pass, copy acceptance still manual. Chrome `Secure Preferences` shows extension `lloidknoffahllnjeemcjmahcbjoohlc` loaded from the current repository `dist`, and `dist/manifest.json` reports `0.3.4 Prompt Contract`. The real Chrome extension history page at `chrome-extension://lloidknoffahllnjeemcjmahcbjoohlc/popup.html#history` loaded with title `Zhijuan Prompt Card`, rendered 105 local records, and the latest user case rendered a successful visible prompt with 395 words starting from portrait frame guidance. The visible prompt contains aspect/orientation cues, does not contain `schema_version` or `reconstruction_v2`, and does not contain Image2/reference-wrapper phrasing. The page also rendered 105 `复制 JSON 提示词` controls. Direct clipboard-copy automation from this user Chrome session remained inconclusive because Apple Events injection could not access `chrome.storage.local`, keyboard activation did not trigger the focused copy button, and coordinate clicking targeted an unrelated system UI; therefore user-installed copy acceptance remains manual.
- Release package: pass, `npm run release:package`.
- Release scan: pass, `npm run release:check`.
- Package output: `release/zhijuan-prompt-card-0.3.4.zip`.
- Package sha256: `064b6b3233f6df39cdcfc24902273028251aca2bf99aa4befaef72d8328f83a1`.
- Post-documentation consistency check: pass. `README.md`, `README.zh-CN.md`, `CHANGELOG.md`, `docs/release-notes-v0.3.4.md`, and `docs/installed-extension-acceptance.md` now describe the v0.3.4 structured generator-facing JSON contract without the old prompt-first JSON wording; the release draft SHA256 matches the package hash above. After the final `npm run build`, `npm run release:package` was rerun so the zip reflects the current built `dist/` plus required notices.
- Post-documentation gates: pass, `npm run check:prompt-goal`, `npm run check:storage`, `npm run check:extension-pages`, `npm run check:content-script`, `npm run release:check`, and `git diff --check`.
- Dist manifest: pass, manifest `version` is `0.3.4`, `version_name` is `0.3.4 Prompt Contract`.
- Zip manifest: pass, manifest `version` is `0.3.4`, `version_name` is `0.3.4 Prompt Contract`.
- Manual zip sensitive scan: pass, 23 files; no matches for private account paths, API-key-like strings, bearer tokens, or local home-directory paths in `release/zhijuan-prompt-card-0.3.4.zip`.
- Current-tree sensitive scan: pass; no matches for private account paths, API-key-like strings, bearer tokens, or local home-directory paths outside ignored build, release, temp, and dependency outputs.
- Diff whitespace: pass, `git diff --check`.
- Public git history gate: fail on pre-existing historical entries, including old BridgeDeck local account URLs and local absolute paths in prior commits. The v0.3.4 release zip and current tree scans are clean, but a strict public-history release should use a clean orphan branch or clean repository history instead of publishing the full existing history.
- Targeted cold review: pass with low-risk submission hygiene note. The final cold review found no blocker in generator JSON shape, style fingerprint guidance, docs, or regression tests; two untracked README image assets remained intentionally unstaged because they were not referenced by current docs.
- Cloud Codex review gate: pending for current local head. PR #5 still points at older remote commit `b516170e28`; the current local candidate is not pushed until user-installed real testing passes.
- v0.3.4 scope: source-frame orientation/crop/aspect preservation, contradictory frame-claim correction, structured generator-facing JSON contract with adaptive modules and atmosphere, internal structured evidence retention, source-style fingerprint guidance, and conditional style/detail negative blockers for better image-to-prompt-to-image reconstruction fidelity.
- Release status: not pushed, not merged, not tagged, and not published. This build stops at user refreshed-extension acceptance before any GitHub release.
...
- Bilibili E2E: pass, `npm run e2e:bilibili`, `tmp/browser-tests/pw-e2e-summary-1782027891136.json`; includes `copy_english_prompt_ok` and `copy_json_ok`, with JSON copy parsed and asserted to put `prompt` first and exclude `schema_version` / `reconstruction_v2`.
- Visual history smoke: pass, `npm run smoke:history`, `tmp/browser-tests/pw-visual-history-summary-1782027867785.json`; 14, 35, and 100 record cases, running-history guard, clear-sync guard, popup history rendering, and visible `复制 JSON 提示词` actions passed.
- Automated real-extension UI test: pass, `npm run test:real-extension`, `tmp/browser-tests/manual-real-extension-summary-1782027119160.json`; loaded the built extension in a temporary Chromium profile, saved real BridgeDeck settings with redacted evidence, then analyzed README page image pick and local file upload through the actual UI. Both returned `reconstruction_v2`, `en.prompt`, `json_prompt.generation_prompt`, `json_prompt.generation_negative_prompt`, `json_prompt.spatial_dynamics`, `json_prompt.fidelity_priorities`, dynamic `observation_units`, dynamic `reconstruction_priorities`, no hidden Japanese output, and no duplicate recreation-prompt field. This does not replace the separate user-installed refreshed-extension acceptance gate. Page image pick returned 375 English prompt words, 445 JSON generation prompt words, 74 spatial dynamics words, 8 observation units, and 5 reconstruction priorities. Local upload returned 328 English prompt words, 472 JSON generation prompt words, 67 spatial dynamics words, 9 observation units, and 5 reconstruction priorities.
- Release package: pass, `npm run release:package`.
- Release scan: pass, `npm run release:check`.
- Package output: `release/zhijuan-prompt-card-0.3.3.zip`.
- Package sha256: `f5f5f9f206c40fe6da225835ac706c9adc077c089e60041e14e07d9dd4c9a39e`.
...
- Release status: pushed to PR branch; pending current-head cloud review with no blockers, user-installed refreshed-extension acceptance, merge, tag, and GitHub release.
